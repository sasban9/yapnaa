<?php 
	...
	if(isset($_POST['tm_interaction'])){
		...
		if(!empty($_POST['transaction_service_experience'])){
			/*$tse_arr 		 	= explode("_",$_POST['transaction_service_experience']);
			$get_cqa_data 		= $obj_user->get_question_answer_for_landing_page($_GET['user_id'],$tse_arr[0]);
			$value_of_qsn 		= $obj_user->get_value_of_answer($get_cqa_data['answer_weightage'],$tse_arr[0]);
			if(empty($value_of_qsn)){
				$value_of_qsn   = 'New';
			}*/
			...
			print_r($existing_category); echo "</br>";
			print_r($selling_customer_category); echo "</br>";
			...
			$response2  = $obj_user->updateProfileInBrand($brand_name,$set_array_brand,$_POST['brand_customer_id'],$_POST['user_id']);
									
		}
		 
		/*if(!empty($_POST['transaction_brand_experience'])){
			$tse_arr 		 	= explode("_",$_POST['transaction_brand_experience']);
			$get_cqa_data 		= $obj_user->get_question_answer_for_landing_page($_GET['user_id'],$tse_arr[0]);
			$value_of_qsn 		= $obj_user->get_value_of_answer($get_cqa_data['answer_weightage'],$tse_arr[0]);
			if(empty($value_of_qsn)){
				$value_of_qsn   = 'New';
			...
			$timeline_data		= array(
										...
										'tm_interaction_type' 	=> $_POST['tm_interaction_type'],
										'tm_customer_comment' 	=> $_POST['transaction_comment'],
										'tm_movement_from' 		=> $value_of_qsn,
										'tm_movement_to' 		=> $tse_arr[3]
										);
			$response 			= $obj_user->insert_transaction_lifecycle_data($timeline_data);	
		}
		if(!empty($_POST['transaction_referal_experience'])){
			$tse_arr 		 	= explode("_",$_POST['transaction_referal_experience']);
			$get_cqa_data 		= $obj_user->get_question_answer_for_landing_page($_GET['user_id'],$tse_arr[0]);
			$value_of_qsn 		= $obj_user->get_value_of_answer($get_cqa_data['answer_weightage'],$tse_arr[0]);
			if(empty($value_of_qsn)){
				$value_of_qsn   = 'New';
