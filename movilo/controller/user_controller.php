...
$headers .= "From: info@yapnaa.com";   
  ...
  function  n_yapnaa_forgot_password_save($user_mobile,$pwd){
	...
	function logout(){
        ...
        echo '<script>
document.location.href = "https://www.google.com/accounts/Logout?continue=https://appengine.google.com/_ah/logout?continue=https://yapnaa.com/login.php";
</script>';		
		echo '<script>window.location.assign("../../login.php")</script>';
	...
	//get product details for dashbord
	function product_info(){
		session_start();
		$user_id=$_SESSION['user_id'];
		$product_info=$this->model->get_product_id($user_id);
		//echo '<pre>';print_r($product_info); 
		return $product_info;
	}
	//get service_info details for dashbord
	function service_info(){
		session_start();
		$user_id=$_SESSION['user_id'];
		$service_info=$this->model->get_service_status($user_id);
		//echo '<pre>';print_r($product_info); 
		return $service_info;
	}
	
	//save service request for dashboard
	function save_request_raise1($issue_t,$brand_type,$brand_name,$issue_type,$serial_numaber,$user_id){
        ...
        $fields_reg1 = array(
					'srm_user_id' 				=> $user_id,					
					'srm_user_notes'			=> $issue_type,
					'srm_product_id'			=> $brand_name,
					'type'                      => $issue_t, 
					'srm_c_date' 				=> date('Y-m-d h:i'),
					'srm_user_generated_date'	=> date('Y-m-d h:i')
				); 
				 //print_r($fields_reg1);exit;
			$table_log_in1 = 'SRM';
			$arr_result1   = $this->model->insert($table_log_in1, $fields_reg1);		
		    $user_name                          =$user_name;
		    $user_id                         =$user_id;
			$subject	=	"Service Requested";
			$this->send_email1($user_name,$user_id,$subject);  
		 
		$fields_reg = array(
                
                'user_id' 			            => $useID,
                'serial_number'                 =>$serial_numaber,
				'product_type'                  =>$brand_info,
				'brand_name'                    =>$brand,
				'issue_type'                    =>$issue,
				'type'                          =>$issue_t,
	...
	//get my ticket dashboard
	function get_request_ticket($issue_t,$brand_info,$brand,$issue,$serial_numaber,$useID){
		
		$fields_user='*';
		$table_user='service_request_raise';
		$condition_user='user_id like "%'.$search_query.'%" or customer_phone like"%'.$phone.'%"';
		$arr_result = $this->model->get_Details_condition($table_user, $fields_user, $condition_user);
			if($arr_result){
				return 1;
			  }
			  else{
...
/*----------------------------------------------------------------------------------------------------------------------*/		
     ...
     function add_product($bId,$product_user_id,$brands,$dateInstallation)
    {
        ...
        $add_product_installation	  	 = 	($dateInstallation !='')?'Yes':'';
...
/*----------------------------------------------------------------------------------------------------------------------*/		
	// Update update_user_profile_dashboard
	function update_user_profile_dashboard()
    {
		session_start();
		//print_r($_POST);//die;
        
        $up_p_name				   		    = 	$_POST['profile_name'];
        $up_p_mobile			   		 	= 	$_POST['profile_phone_number'];
        $up_p_email				   	 	    = 	$_POST['profile_email_id'];
        $up_p_user_id				   	 	= 	$_SESSION['user_id'];
        $fields_reg = array(); 
        if(!empty($up_p_mobile)){$fields_reg['user_phone']	=	$up_p_mobile;}		
		if(!empty($up_p_name)){$fields_reg['user_name']	=	$up_p_name;}
		if(!empty($up_p_email)){$fields_reg['user_email_id']	=	$up_p_email;}	
        //print_r($fields_reg);die;		
		$table_log_in = "users";
...
// Update user profile
	...
	function n_yapnaa_feedback($user_id,$message,$yapnaarate)
    {
        $table           					 = 	table(); 
        
        $fb_user_id				   		 	= 	$user_id;
        $fb_rating					   	 	= 	$yapnaarate;
        $fb_message					   		= 	$message; 
		
		$table_log_in    				    = 	$table['tb1'];
        $fields_log_in   				    = 	'*'; 
        $condition_log_in				    =  "user_id= $fb_user_id";
        $arr_log_in      	 			  = 	$this->model->get_Details_condition($table_log_in, $fields_log_in, $condition_log_in);
		// print_r($arr_log_in);exit;
		...
		if($arr_log_in){  
            ...
            $arr_result   = $this->model->insert($table_log_in, $fields_reg); 
			echo'<script>alert("Thank you for giving you valuable feedback.")</script>';
...
/*----------------------------------------------------------------------------------------------------------------------*/		
	...
	function send_email1($user_name,$user_id,$subject){
		$message	=	'<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
   <head>
      <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
                                       ...
                                       <table class="one-column" border="0" cellpadding="40" cellspacing="0" width="100%" style="border-spacing:0" bgcolor="#FFFFFF">
                                          <tr>
                                             <td align="" style="padding-bottom:2%; padding-top:2%;">
												...
												<p style="color:#666666; font-size:14px;  font-family: "Montserrat", sans-serif;"><b>User Id:'.$user_id.'</b></p>
                                                <!-- START BUTTON -->
                                                <!-- END BUTTON -->
                                             </td>
                                          </tr>
