...
function calculate_profile($group_subgroup_result){
	...
	foreach($group_subgroup_result as $key4 => $value4){
		...
		if($key4 == 'Customer Satisfaction Index'){
			$cscounter 		= 0;
			$cscounter 		= array_sum(array_column($value4,'answer_weightage'));
		...
		if($key4 == 'Customer Brand Engagement Index'){
			$cbecounter 	= 0;
			$cbecounter 	= array_sum(array_column($value4,'answer_weightage'));
		...
		if($key4 == 'Customer Referal Value Index'){
			$crvcounter 	= 0;
			$crvcounter 	= array_sum(array_column($value4,'answer_weightage'));
	...
	//echo $cs_customer_stats; echo 'suman'; echo $cbe_customer_stats; echo 'kumar'; echo $crv_customer_stats;die;
	...
	//echo "<br><pre>"; print_r($response_array); die;						
