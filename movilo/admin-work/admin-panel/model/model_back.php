...
"; 
		...
		(SELECT user_phone FROM users WHERE user_phone = zc.phone1 or user_phone = zc.phone2 
		GROUP by zc.CUSTOMERID) AS users FROM $table zc where 
		 zc.phone1 not in (select user_phone from user_question_aws_mapping where user_phone=zc.phone1) and (zc.last_call_comment ='' or zc.last_call_comment=NULL) limit 100";
		$qry	=	connection()->query($sql);				
		}	
		$ret=array();
		...
		while($row=mysqli_fetch_assoc($qry)){
			  ...
			  $row1=mysqli_fetch_all($qry1,MYSQLI_ASSOC);
			  if($row1 !=NULL){
			   $row['qust_map']=$row1;
			  }else{
				  $row['qust_map']=array();
