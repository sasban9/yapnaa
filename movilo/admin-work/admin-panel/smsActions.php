...
if(isset($_SESSION['admin_email_id'])){
	...
	if(isset($_REQUEST['expiryDate'])){
		...
		$id = $_REQUEST['id'];
		//echo $my_date."--".$number;
		$get_amc_list = $control->zerob_appointment_expiry_sms($id,$date,$number);
		echo 1;
		exit;
	}
	if(isset($_REQUEST['notInterested'])){
		$id = $_REQUEST['id'];
		$comment = $_REQUEST['comment'];
		$get_amc_list = $control->notInterested($id,$comment);
		echo 1;
		exit;
	}
	if(isset($_REQUEST['general'])){
		...
		$get_amc_list = $control->send_user_sms($number,$message,$id);
		echo 1;
		exit;
	}
