...
/* Get product List*/
$get_business_store_product_list	=	$admin_controller->get_all_product_list();  
...
{	
	...
	if(!empty($add_store_product)){
		...
		echo '<script>window.location.assign("product-list.php")</script>';
	} 
}
								 ...
								 <table class="table table-striped m-b-none" id="mc_table">
									...
									<?php for($i=0;$i<count($get_business_store_product_list);$i++){  ?>
										<tr>
										    ...
										    <td><a href="product_list_details.php?p_id=<?php echo $get_business_store_product_list[$i]['p_id']; ?>" target="_blank">View Details</td> 
